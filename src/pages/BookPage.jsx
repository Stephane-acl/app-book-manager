    ...
    const fetchBook = async id => {
        try {
            ...
            const {
                ...
                isAvailable,
                author
            } = await API.findBook(id);

            setBook({title, nbrPages, language, description, dateOfPublication, image, isAvailable, author: author.id});
            setLoading(false);
        } catch (error) {
        ...
        <>
            ...
            {!loading && (
                <form>
                    ...
                    />
                    <Select name='author' label='Auteur' value={book.author} error={errors.book}
                            onChange={handleChange}>
                        {authors.map(author =>
            ...
            {!editing && <Link to='/authors/new' type='submit' className="btn btn-info">Création d'un auteur</Link>}
